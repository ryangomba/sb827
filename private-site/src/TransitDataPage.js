...
function makeSorter(key) {
  return (a, b) => {
    ...
    const akInt = parseInt(a[key], 10);
    const bkInt = parseInt(b[key], 10);
...
class TransitDataPage extends React.Component {
  ...
  _onRouteChange = (e, data) => {
    ...
    if (routeId !== allRoutesId) {
      Object.keys(
        agencyData[agencyIdx].debugDetails[dateType][routeId].directions
      ).forEach(directionId => {
  ...
  _getRouteRows() {
    ...
    const { agencyIdx, dateType } = this.state;
    return Object.values(agencyData[agencyIdx].debugDetails[dateType]).sort(
      makeSorter("name")
    );
  }
  ...
  _getStopRows() {
    ...
    const agency = agencyData[agencyIdx];
    return Object.entries(
      agency.debugDetails[dateType][routeId].directions[directionId].stops
      ...
      <div>
        ...
        <div className="tables">
          <Table celled structured>
            <Table.Header>
              <Table.Row>
                <Table.HeaderCell rowSpan="2">{rowHeader}</Table.HeaderCell>
                <Table.HeaderCell
